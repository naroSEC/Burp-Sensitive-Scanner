...
Scan의 raw message 보관 한도를 넘은 finding은 선택할 때 transaction fingerprint를 사용해 Proxy History, Site Map, live capture 또는 imported Logger repository에서 원문을 다시 가져옵니다. 원본 항목이 이미 제거된 경우 editor를 비우고 Finding Details에 unavailable 상태를 표시하므로, 직전에 선택한 다른 request가 남아 보이지 않습니다.
