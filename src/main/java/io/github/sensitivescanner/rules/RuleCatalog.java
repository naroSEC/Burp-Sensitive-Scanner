...
public final class RuleCatalog {
    ...
    public static List<DetectionRule> defaults(){
        ...
        r.add(info("SENSITIVE-ENV-REFERENCE","Environment Configuration File","Sensitive File",Severity.MEDIUM,"(?<![A-Za-z0-9_])(\\.env(?:\\.[A-Za-z0-9_-]+)?)(?![A-Za-z0-9_-])",ALL));
        ...
        r.add(new RegexRule("SENSITIVE-GENERIC-API-KEY","Generic API Key","Generic","API key identified by nearby key context and value format",Severity.MEDIUM,"(?:(?:\"?)api(?:Key|_key|[\\s.-]{1,5}key)(?:\"?)[\\s]*[:=][\\s]*[\"']?([^\"'&\\s,;}]{10,512})|api.{0,5}key[^&|;?,\\r\\n]{0,32}?[\"']([A-Za-z0-9_\\-+=/\\\\]{10,512})[\"'])",CI,(v,s)->genericConfidence(v,s),ALL,0));
        r.add(new RegexRule("SENSITIVE-GENERIC-SECRET","Generic Secret","Generic","Sensitive key paired with a non-placeholder high-entropy value",Severity.MEDIUM,"(?:(?:\"?)(?:password|passwd|pwd|secret(?:Key|_key)?|api(?:Secret|_secret)|client(?:Secret|_secret)|access(?:Token|_token)|refresh(?:Token|_token)|auth(?:Token|_token)|session(?:Token|_token))(?:\"?)[\\s]*[:=][\\s]*[\"']?([^\"'&\\s,;}]{8,512})|secret[^&|;?,\\r\\n]{0,32}?[\"']([A-Za-z0-9_\\-+=/\\\\]{10,512})[\"'])",CI,(v,s)->genericConfidence(v,s),ALL,0));
        ...
        r.add(info("SENSITIVE-LOOPBACK-IP","Loopback IPv4 Address","Infrastructure",Severity.LOW,"(?<![0-9])((?:127(?:\\.(?:25[0-5]|2[0-4]\\d|1?\\d?\\d)){3}))(?![0-9])",ALL));
    ...
    private static Confidence genericConfidence(String v,io.github.sensitivescanner.scanner.ScanSettings s){return validGeneric(v,s)?(SecretUtils.entropy(v)>=s.entropyThreshold?Confidence.HIGH:Confidence.MEDIUM):null;}
