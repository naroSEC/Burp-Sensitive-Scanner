...
public final class SensitiveScannerExtension implements BurpExtension {
    @Override public void initialize(MontoyaApi api){
        api.extension().setName("Sensitive Scanner");TrafficRepository capturedRepository=new TrafficRepository(20000),importedRepository=new TrafficRepository(500000);AtomicReference<SensitiveScannerPanel> panelRef=new AtomicReference<>();AtomicReference<Registration> tabRef=new AtomicReference<>();
