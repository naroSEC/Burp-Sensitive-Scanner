...
public final class SensitiveScannerPanel extends JPanel {
    ...
    private volatile TrafficTransaction activeTraffic;
    ...
    private void showFinding(Finding finding){
        activeFinding=finding;activeTraffic=null;long sequence=++selectionSequence;clearEditors();TrafficTransaction retained=finding.traffic();
        if(retained.messageBytes()>0){renderTraffic(finding,retained);details.setText(detailsText(finding,"retained from scan"));details.setCaretPosition(0);return;}
        details.setText(detailsText(finding,"restoring from "+retained.source()+"…"));details.setCaretPosition(0);
        new SwingWorker<TrafficTransaction,Void>(){
            protected TrafficTransaction doInBackground(){return collector.resolve(retained,finding.trafficKey()).orElse(null);}
            protected void done(){if(sequence!=selectionSequence||activeFinding!=finding)return;try{TrafficTransaction resolved=get();if(resolved==null){details.setText(detailsText(finding,"unavailable; the source entry may have been removed"));details.setCaretPosition(0);return;}renderTraffic(finding,resolved);details.setText(detailsText(finding,"restored from "+retained.source()));details.setCaretPosition(0);}catch(Exception error){api.logging().logToError("Could not restore finding traffic",error);details.setText(detailsText(finding,"restore failed; see Extension errors"));details.setCaretPosition(0);}}
        }.execute();
    }
    private void clearEditors(){try{responseEditor.setSearchExpression("");requestEditor.setRequest(HttpRequest.httpRequest());responseEditor.setResponse(HttpResponse.httpResponse());}catch(RuntimeException error){api.logging().logToError("Could not clear HTTP message editors",error);}}
    private void renderTraffic(Finding finding,TrafficTransaction traffic){try{HttpService service=HttpService.httpService(traffic.host(),traffic.port(),traffic.tls());requestEditor.setRequest(traffic.requestLength()>0?HttpRequest.httpRequest(service,ByteArray.byteArray(traffic.request())):HttpRequest.httpRequest());responseEditor.setResponse(traffic.responseLength()>0?HttpResponse.httpResponse(ByteArray.byteArray(traffic.response())):HttpResponse.httpResponse());activeTraffic=traffic;if(messageTabs.getSelectedIndex()==1)SwingUtilities.invokeLater(()->{if(activeFinding==finding&&activeTraffic==traffic)focusResponseMatch(finding,traffic);});}catch(RuntimeException error){api.logging().logToError("Could not render selected HTTP message",error);clearEditors();}}
    private String detailsText(Finding f,String rawState){return "Rule ID: "+f.ruleId()+"\nType: "+f.type()+"\nCategory: "+f.category()+"\nSeverity: "+f.severity()+"\nConfidence: "+f.confidence()+"\nMatch: "+f.evidence()+"\nURL: "+f.traffic().url()+"\nSection: "+f.location()+"\nField: "+f.fieldName()+"\nDetection path: "+f.detectionPath()+"\nSource: "+f.traffic().source()+" / "+f.traffic().tool()+"\nFirst seen: "+f.firstSeen()+"\nOccurrence count: "+f.occurrences()+"\nRaw message: "+rawState;}
    private void focusResponseMatch(Finding finding,TrafficTransaction traffic){if(!isResponseFinding(finding)||traffic.responseLength()==0){responseEditor.setSearchExpression("");return;}try{ResponseMatchLocator.Target target=ResponseMatchLocator.locate(traffic.response(),finding.evidence(),finding.fieldName());if(target==null){responseEditor.setSearchExpression("");return;}responseEditor.setSearchExpression(target.expression());responseEditor.setCaretPosition(target.offset());responseEditor.uiComponent().requestFocusInWindow();}catch(RuntimeException e){api.logging().logToError("Could not focus the detected response match",e);}}
